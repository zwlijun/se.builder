exports.init = function(grunt) {
  ...
  var _ = grunt.util._ = require("lodash");
  ...
  function jsParser(fileObj, options) {
    ...
    var file = getFileInfo(fileObj.src);
    ...
    function getDeps(file, fn) {
      return file.dependencies.map(function(dep) {
        ...
        // console.log(dep);
        // console.log("---------------------------")
        // console.log(Object.prototype.toString.call(dep) + ">>>>")
        // console.log("\n\n\n");
        // console.log("---------------------------")
        var _typeof = Object.prototype.toString.call(dep);

        if("[object Object]" == _typeof){
            dep = fn ? fn(dep) : dep.id;
        }else if("[object Array]" == _typeof){
            //todo
        }
    ...
    function parseModuleDependencies(id) {
      ...
      var deps = parsed.map(function(meta) {
        return meta.dependencies.map(function(id) {
          ...
          if (!_.includes(ids, id) && !_.includes(ids, id.replace(/\.js$/, ''))) {
            ...
            id = iduri.appendext(id);
            // console.log("id2: " + id)
            // console.log("base: " + fbase);
            // console.log("join: " + path.join(fbase, id));
      ...
      var sn = function(items, ret){
        ...
        for(var i = 0; i < size; i++){
          ...
          __type = Object.prototype.toString.call(item);

          if("[object Array]" == __type){
            ret = sn(item, ret);
          }else{
            ret.push(item);
          }
